                        ...
                        if status:
                            ...
                            if status == "paid":
                                ...
                                if user_id:
                                    ...
                                    if invoice_data.get("amount", 0) >= 3:
                                        ...
                                        try:
                                            ...
                                            logger.info(f"Поддержка от пользователя {user_id}: {invoice_data.get('amount')} {invoice_data.get('currency')}")
                                        except Exception as e:
...
def simple_donate_handler(call):
    ...
    keyboard.add(
        ...
        types.InlineKeyboardButton("❤️ 20 USDT", callback_data="donate_amount:20"),
...
def handle_donate_amount_input(user_id, amount_text):
...
def handle_text_input(message):
    ...
    elif current_state == "entering_donate_amount":
        success = handle_donate_amount_input(user_id, text)
        if success:
            UserState.set_state(user_id, None)
    
    elif current_state == "admin_replying":
        handle_admin_reply_text(user_id, text)
...
# ===== ИСПРАВЛЕННЫЙ МЕХАНИЗМ ОТВЕТА АДМИНИСТРАТОРА =====
def handle_admin_reply_text(user_id, reply_text):
    """Обработка текста ответа администратора"""
    # Получаем данные состояния
    state_data = UserState.get_data(user_id)
...
"""
        ...
        # Пытаемся отправить через safe_send_message
        msg = safe_send_message(int(target_user_id), response_text)
        ...
        if msg:
            ...
            logger.error(f"Не удалось отправить ответ от админа {user_id} пользователю {target_user_id}")
    
    except Exception as e:
        # Обработка исключений при отправке
        logger.error(f"Ошибка отправки ответа администратора: {e}")
        safe_send_message(user_id,
            f"❌ <b>Ошибка при отправке ответа:</b> {str(e)[:100]}",
            reply_markup=get_main_keyboard()
        )
...
# ===== МЕХАНИЗМ РАССЫЛКИ ДЛЯ АДМИНИСТРАТОРОВ =====
def handle_admin_broadcast(user_id, broadcast_text):
    """Обработка рассылки сообщения всем пользователям"""
    if user_id not in ADMIN_IDS:
        safe_send_message(user_id, "❌ Доступ запрещен", reply_markup=get_main_keyboard())
        return
    ...
    UserState.update_data(user_id, "broadcast_text", broadcast_text)
    
    # Отправляем подтверждение перед началом рассылки
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        types.InlineKeyboardButton("✅ Начать рассылку", callback_data=f"confirm_broadcast:{hashlib.md5(broadcast_text.encode()).hexdigest()[:8]}"),
...
def my_payments_handler(call):
    ...
    user_invoices = []
    for invoice_id, invoice_data in storage.invoices.items():
        if str(invoice_data.get("user_id")) == str(user_id):
            user_invoices.append((invoice_id, invoice_data))
    ...
    # Сортируем по дате создания (новые сначала)
    user_invoices.sort(key=lambda x: x[1].get("created_at", datetime.min), reverse=True)
    
    # Формируем сообщение
    payments_text = "📋 <b>История ваших платежей:</b>\n\n"
    
    for invoice_id, invoice_data in user_invoices:
    ...
    payments_text += (
        f"\n📊 <b>Всего платежей:</b> {len(user_invoices)}\n"
        f"💰 <b>Оплачено:</b> {sum(i[1].get('amount', 0) for i in user_invoices if i[1].get('status') == 'paid')} USDT"
    )
...
@bot.message_handler(content_types=['photo', 'video', 'document'], func=lambda m: UserState.get_state(m.from_user.id) == "admin_broadcast")
def handle_broadcast_media(message):
    """Обработка медиа-контента для рассылки"""
    user_id = message.from_user.id
    ...
    safe_send_message(user_id,
        "❌ <b>Медиа-рассылка пока не поддерживается.</b>\n\n"
        "Пожалуйста, отправьте текстовое сообщение для рассылки.",
        reply_markup=get_cancel_keyboard()
    )

@bot.message_handler(commands=['broadcast_text'])
def broadcast_text_command(message):
    ...
    if len(command_parts) < 2:
        bot.send_message(user_id,
            "📝 <b>Использование:</b> /broadcast_text <i>текст сообщения</i>\n\n"
            "Пример: /broadcast_text Важное обновление бота!",
            reply_markup=get_main_keyboard()
        )
        return
    ...
    # Сохраняем текст в состоянии
    UserState.set_state(user_id, "admin_broadcast", {"broadcast_text": broadcast_text})
    ...
    bot.send_message(
        user_id,
        f"📢 <b>Подтверждение быстрой рассылки</b>\n\n"
        f"Текст сообщения:\n━━━━━━━━━━━━\n{broadcast_text}\n━━━━━━━━━━━━\n\n"
        f"Количество получателей: <b>{len(storage.users)}</b>\n\n"
        "Начать рассылку?",
        reply_markup=keyboard
    )
