...
def get_condition_keyboard():
    ...
    keyboard.add(types.InlineKeyboardButton("🔙 Назад", callback_data="universal_back"))
    return keyboard
...
def get_models_keyboard(page=0, search_query=""):
    ...
    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton("⬅️ Предыдущая", 
                     callback_data=f"models_page:{page-1}:{search_query}"))
    if end_idx < len(filtered):
        nav_buttons.append(types.InlineKeyboardButton("Следующая ➡️", 
                     callback_data=f"models_page:{page+1}:{search_query}"))
    ...
    keyboard.add(
        ...
        types.InlineKeyboardButton("🔙 Назад", callback_data="universal_back")
    )
...
def reset_user_state(user_id):
    ...
    if user_id in storage.states:
        ...
        # ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ: Гарантируем показ основной клавиатуры
...
def ensure_main_keyboard(user_id):
    ...
    try:
        # Отправляем пустое сообщение с клавиатуром
...
def clear_ad_creation_state(user_id):
    """Полная очистка состояния перед началом создания объявления"""
    # Удаляем черновик, если есть
    if user_id in storage.drafts:
        del storage.drafts[user_id]
    
    # Очищаем состояние, если оно связано с созданием объявления
    if user_id in storage.states:
        current_state = storage.states[user_id].get('state')
        if current_state and current_state in [
            "select_model", "select_condition", "select_memory", 
            "entering_memory", "entering_color", "select_box", 
            "select_docs", "select_accessories", "entering_accessories",
            "entering_price", "entering_city", "entering_metro",
            "uploading_photos", "preview_ad", "searching_model",
            "entering_custom_model"
        ]:
            del storage.states[user_id]
    
    # Очищаем кэш сообщений для этого пользователя
    keys_to_remove = []
    for key in storage.message_cache:
        if key[0] == user_id:
    ...
    logger.info(f"Очищено состояние создания объявления для пользователя {user_id}")
    return True

def complete_ad_creation(user_id):
    """Корректное завершение процесса создания объявления"""
    # Очищаем состояние
    if user_id in storage.states:
        del storage.states[user_id]
    ...
    logger.info(f"Завершен процесс создания объявления для пользователя {user_id}")

def check_state_conflict(user_id, expected_state=None):
    """Проверка конфликта состояний"""
    current_state = UserState.get_state(user_id)
    
    # Если есть активное состояние, не связанное с созданием объявления
    if current_state and current_state not in [
        ...
        "entering_price", "entering_city", "entering_metro",
        "uploading_photos", "preview_ad", "searching_model",
        "entering_custom_model", None
    ]:
        logger.warning(f"Конфликт состояний у пользователя {user_id}: текущее {current_state}, ожидаемое {expected_state}")
        # Сбрасываем конфликтующее состояние
        UserState.set_state(user_id, None)
        return True
    return False

# ===== ФУНКЦИИ ПОКАЗА ПРЕВЬЮ =====
def show_ad_preview(user_id):
    ...
    preview_text = f"""
📱 <b>Модель:</b> {draft.get('model', 'Не указано')}
📊 <b>Состояние:</b> {draft.get('condition', 'Не указано')}
💾 <b>Память:</b> {draft.get('memory', 'Не указана')}
🎨 <b>Цвет:</b> {draft.get('color', 'Не указан')}
📦 <b>Коробка:</b> {'✅ Да' if draft.get('has_box') else '❌ Нет'}
📄 <b>Документы:</b> {'✅ Да' if draft.get('has_docs') else '❌ Нет'}
🔧 <b>Комплектация:</b> {draft.get('accessories', 'Не указана')}
💰 <b>Цена:</b> {draft.get('price', 0):,} сом
📍 <b>Местоположение:</b> {draft.get('city', 'Не указан')}
🚇 <b>Метро:</b> {draft.get('metro', 'Не указано')}
📸 <b>Фотографий:</b> {len(draft.get('photos', []))}
"""
    ...
    # Показываем кнопки действий с кнопкой Назад
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
...
# ===== ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ОБРАБОТКИ CALLBACK =====
def photos_done_handler(call):
    ...
    # Убедимся, что состояние корректно установлено
...
def contact_seller(call):
    """Обработка связи с продавцом - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
    ...
    # Предлагаем способы связи с кнопкой Назад
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
...
def delete_advertisement(call):
    """Удаление объявления - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
    ad_id = call.data.split(':')[1]
    ...
    # Запрашиваем подтверждение с кнопкой Назад
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
...
def show_ad_stats(call):
    """Показать статистику объявления - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
    ad_id = call.data.split(':')[1]
...
"""
    ...
    bot.send_message(user_id, stats_text, reply_markup=keyboard)
    bot.answer_callback_query(call.id)
...
def cancel_draft(call):
    ...
    # ВЫЗОВ ЗАВЕРШЕНИЯ: Корректно завершаем процесс
    complete_ad_creation(user_id)
    
    bot.answer_callback_query(call.id)

def edit_draft(call):
    """Редактировать черновик - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
    draft = AdDraftManager.get_draft(user_id)
    ...
    # Добавляем кнопку Назад
    keyboard.add(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_preview"))
...
def confirm_broadcast_handler(call):
    ...
    broadcast_hash = call.data.split(':')[1]
    ...
    if hashlib.md5(broadcast_text.encode()).hexdigest()[:8] != broadcast_hash:
        bot.answer_callback_query(call.id, "❌ Ошибка проверки данных", show_alert=True)
        return
    ...
    progress_msg = bot.send_message(admin_id, 
    ...
    # Добавляем подпись к рассылке
    full_message = f"{broadcast_text}\n\n━━━━━━━━━━━━\n💬 <i>Рассылка от администрации</i>"
    
    for i, (user_id, user_data) in enumerate(storage.users.items(), 1):
        try:
            ...
            result = safe_send_message(int(user_id), full_message)
            
            if result:
                success_count += 1
            else:
                error_count += 1
                logger.warning(f"Не удалось отправить рассылку пользователю {user_id}")
...
• Процент доставки: {(success_count/total_users*100):.1f}%

⏱ <b>Время выполнения:</b> {datetime.now().strftime('%H:%M:%S')}
"""
    
    safe_send_message(admin_id, report_text, reply_markup=get_main_keyboard())
    
    # Сбрасываем состояние
    UserState.set_state(admin_id, None)
    
    # Логируем результат
    logger.info(f"Админ {admin_id} выполнил рассылку: {success_count}/{total_users} успешно")
...
# ===== ДОБАВЛЯЕМ ВСЕДЕЙСТВУЮЩИЙ BACK HANDLER =====
@bot.callback_query_handler(func=lambda call: call.data == "universal_back")
def universal_back_handler(call):
    ...
    # Используем существующий back_handler для состояний, которые он обрабатывает
    if current_state in ["select_condition", "select_memory", "entering_memory", 
                        "entering_color", "select_box", "select_docs", 
                        "select_accessories", "entering_accessories", 
                        "entering_price", "entering_city", "entering_metro", 
                        "uploading_photos", "preview_ad"]:
        back_handler(call)
        return
    
    # Специальная обработка для админских состояний
    if current_state == "admin_replying":
        UserState.set_state(user_id, None)
        safe_send_message(user_id, "❌ Ответ отменен. Возврат в главное меню.")
        bot.answer_callback_query(call.id)
        return
    
    if current_state == "waiting_support":
        UserState.set_state(user_id, None)
        safe_send_message(user_id, "❌ Сообщение в поддержку отменено.")
        bot.answer_callback_query(call.id)
        return
    
    if current_state in ["entering_donate_amount", "waiting_donate_amount"]:
        UserState.set_state(user_id, None)
    ...
    if current_state == "entering_custom_model":
        UserState.set_state(user_id, "select_model")
        bot.send_message(user_id, "📱 Выберите модель телефона:", 
                        reply_markup=get_cancel_keyboard())
        bot.send_message(user_id, "Доступные модели:", 
    ...
    # Для всех остальных состояний - просто сбрасываем и показываем главное меню
    UserState.set_state(user_id, None)
    ensure_main_keyboard(user_id)
    bot.answer_callback_query(call.id, "✅ Возврат в главное меню")

def select_memory_handler(call):
    """Обработка выбора памяти - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
...
def support_done_handler(call):
    """Обработка пометки поддержки как решенной"""
    admin_id = call.from_user.id
    ...
    if target_user_id in storage.support_messages:
        ...
        bot.answer_callback_query(call.id, "✅ Помечено как решенное", show_alert=True)
    else:
...
class CryptoBotAPI:
    ...
    def get_invoice(invoice_id):
        ...
        return storage.invoices.get(invoice_id)
...
def start_command(message):
    ...
    # СБРОС: Очищаем возможные конфликтные состояния
    clear_ad_creation_state(user_id)
...
def simple_donate_handler(call):
    ...
    UserState.set_state(user_id, "waiting_donate_amount")
...
def back_to_donate_handler(call):
    """Возврат к меню доната"""
    user_id = call.from_user.id
    UserState.set_state(user_id, None)
    
    # Вызываем существующую функцию donate_command
    donate_command(call.message)
    bot.answer_callback_query(call.id)
...
@bot.callback_query_handler(func=lambda call: call.data == "my_payments")
def my_payments_handler(call):
    """Показать историю платежей пользователя - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
    
    # Ищем все инвойсы пользователя
    user_invoices = storage.user_invoices.get(str(user_id), [])
    ...
    if not user_invoices:
        ...
        bot.send_message(user_id, 
            ...
            "Здесь будут отображаться все ваши платежи через CryptoBot.",
            reply_markup=keyboard
        )
        bot.answer_callback_query(call.id)
        return
    
    # Собираем данные инвойсов
    invoices_data = []
    for invoice_id in user_invoices:
        invoice_data = storage.invoices.get(invoice_id)
        if invoice_data:
            invoices_data.append((invoice_id, invoice_data))
    
    # Сортируем по дате создания (новые сначала)
    invoices_data.sort(key=lambda x: x[1].get("created_at", datetime.min), reverse=True)
    
    # Формируем сообщение
    payments_text = "📋 <b>История ваших платежей:</b>\n\n"
    
    for invoice_id, invoice_data in invoices_data:
        ...
        payments_text += (
            f"<b>Платеж:</b> <code>{invoice_id[:8]}...</code>\n"
            f"<b>Дата:</b> {created_at.strftime('%d.%m.%Y %H:%M')}\n"
            f"<b>Сумма:</b> {amount} {currency}\n"
            f"<b>Статус:</b> {status_display}\n"
        )
        
        # Добавляем информацию о премиуме, если платеж был за премиум
        if amount >= 3 and status == "paid":  # 3 USDT - цена премиума
            payments_text += "🎯 <i>Активирован PREMIUM статус</i>\n"
        
        payments_text += "─" * 30 + "\n"
    
    paid_amount = sum(i[1].get('amount', 0) for i in invoices_data if i[1].get('status') == 'paid')
    payments_text += (
        f"\n📊 <b>Всего платежей:</b> {len(invoices_data)}\n"
        f"💰 <b>Оплачено:</b> {paid_amount} USDT"
    )
    ...
    bot.send_message(user_id, payments_text, reply_markup=keyboard)
    bot.answer_callback_query(call.id)

@bot.message_handler(func=lambda m: m.text == "💎 Донат")
def donate_command(message):
    """Обработка команды доната - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = message.from_user.id
...
📱 <b>Модель:</b> {ad.get('model', 'Не указано')}
💰 <b>Цена:</b> {ad.get('price', 0):,} сом
📍 <b>Местоположение:</b> {ad.get('city', 'Не указан')}
👁 <b>Просмотры:</b> {ad.get('views', 0)}
📅 <b>Опубликовано:</b> {datetime.fromisoformat(ad['created_at']).strftime('%d.%m.%Y %H:%M')}
💎 <b>Статус:</b> {'PREMIUM ✅' if ad.get('is_premium') else 'Обычный'}
"""
...
def show_faq(call):
    """Показать FAQ - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
...
"""
    ...
    safe_send_message(user_id, faq_text, reply_markup=keyboard)
    bot.answer_callback_query(call.id)
...
def handle_text_input(message):
    ...
    # Явная обработка кнопки "Создать объявление"
    if text == "📱 Создать объявление":
        start_ad_creation(message)
        return
    ...
    if text == "❌ Отмена":
        if current_state:
            UserState.set_state(user_id, None)
            safe_send_message(user_id, "❌ Действие отменено. Возвращаю в главное меню.")
        else:
            ensure_main_keyboard(user_id)
        return
    
    # Если пользователь хочет вернуться
    if text == "🔙 Назад" or text == "назад" or text == "Назад":
        universal_back_handler(type('Callback', (), {'from_user': type('User', (), {'id': user_id})(), 'id': 'temp', 'data': 'universal_back'})())
        return
...
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) in [
    "entering_memory", "entering_color", "entering_accessories", 
    "entering_price", "entering_city", "entering_metro", 
    "entering_custom_model", "entering_donate_amount", "searching_model",
    "waiting_support", "admin_replying", "entering_donate_amount",
    "waiting_donate_amount"
])
def handle_text_input_states(message):
    """Обработка текстовых сообщений для состояний ввода - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = message.from_user.id
    current_state = UserState.get_state(user_id)
    text = message.text.strip()
    ...
    # Если пользователь хочет вернуться
    if text == "🔙 Назад" or text == "назад" or text == "Назад":
        universal_back_handler(type('Callback', (), {
    ...
    # Если состояние уже обрабатывается другими хендлерами, пропускаем
    if current_state in ["waiting_support", "admin_replying"]:
        return
    
    # Для состояния поиска модели
    if current_state == "searching_model":
        if len(text) < 2:
            keyboard = types.InlineKeyboardMarkup()
            keyboard.add(types.InlineKeyboardButton("🔙 Назад", callback_data="universal_back"))
        ...
        if found_models:
            ...
            bot.send_message(user_id, 
                f"🔍 <b>Результаты поиска по запросу</b> '{text}':",
                reply_markup=get_cancel_keyboard()
            )
            bot.send_message(user_id, "Найденные модели:", reply_markup=get_models_keyboard(0, text))
        else:
            keyboard = types.InlineKeyboardMarkup()
            keyboard.add(types.InlineKeyboardButton("🔙 Назад", callback_data="universal_back"))
            
            safe_send_message(user_id,
                f"🔍 <b>По запросу '{text}' ничего не найдено.</b>\n\n"
    ...
    # Для других состояний - передаем обработку соответствующим хендлерам
    elif current_state in ["entering_memory", "entering_color", "entering_custom_model", 
                          "entering_accessories", "entering_price", "entering_city", 
                          "entering_metro", "entering_donate_amount"]:
        # Эти состояния обрабатываются другими хендлерами
        pass

# ===== ОБРАБОТКА ТЕКСТОВЫХ СООБЩЕНИЙ ДЛЯ СОСТОЯНИЙ ВВОДА =====
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_memory")
def handle_memory_input(message):
    """Обработка ввода объема памяти"""
    user_id = message.from_user.id
    memory_text = message.text.strip()
    
    if memory_text == "❌ Отмена":
        UserState.set_state(user_id, None)
        ensure_main_keyboard(user_id)
        return
    
    # Сохраняем память в черновик
    AdDraftManager.update_draft(user_id, 'memory', memory_text)
    
    # Переходим к вводу цвета
    UserState.set_state(user_id, "entering_color")
    
    safe_send_message(user_id,
        f"💾 <b>Объем памяти сохранен:</b> {memory_text}\n\n"
        "🎨 <b>Теперь введите цвет телефона:</b>\n\n"
        "Например: <code>Черный</code>, <code>Белый</code>, <code>Синий</code>",
        reply_markup=get_cancel_keyboard()
    )

@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_color")
def handle_color_input(message):
    """Обработка ввода цвета"""
    user_id = message.from_user.id
    color_text = message.text.strip()
    
    if color_text == "❌ Отмена":
        UserState.set_state(user_id, None)
        ensure_main_keyboard(user_id)
        return
    
    # Сохраняем цвет в черновик
    AdDraftManager.update_draft(user_id, 'color', color_text)
    
    # Переходим к выбору коробки
    UserState.set_state(user_id, "select_box")
    
    safe_send_message(user_id,
        f"🎨 <b>Цвет сохранен:</b> {color_text}\n\n"
        "📦 <b>Есть ли оригинальная коробка?</b>\n\n"
        "Укажите, есть ли у вас оригинальная коробка от телефона.",
        reply_markup=get_cancel_keyboard()
    )
...
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_custom_model")
def handle_custom_model_input(message):
    """Обработка ввода пользовательской модели"""
    user_id = message.from_user.id
    model_text = message.text.strip()
    
    if model_text == "❌ Отмена":
        UserState.set_state(user_id, "select_model")
        bot.send_message(user_id, "📱 Выберите модель телефона:", 
                        reply_markup=get_cancel_keyboard())
        bot.send_message(user_id, "Доступные модели:", 
    ...
    AdDraftManager.update_draft(user_id, 'model', model_text)
    
    # Переходим к выбору состояния
    UserState.set_state(user_id, "select_condition")
    
    safe_send_message(user_id,
        f"📱 <b>Модель сохранена:</b> {model_text}\n\n"
...
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_accessories")
def handle_accessories_input(message):
    """Обработка ввода комплектации"""
    user_id = message.from_user.id
    accessories_text = message.text.strip()
    
    if accessories_text == "❌ Отмена":
        UserState.set_state(user_id, None)
        ensure_main_keyboard(user_id)
        return
    
    # Сохраняем комплектацию в черновик
    AdDraftManager.update_draft(user_id, 'accessories', accessories_text)
    
    # Переходим к вводу цены
    UserState.set_state(user_id, "entering_price")
    
    safe_send_message(user_id,
        f"🔧 <b>Комплектация сохранена:</b> {accessories_text}\n\n"
        "💰 <b>Введите цену в сомах:</b>\n\n"
        "Укажите только цифры, без пробелов и символов.\n"
        "Например: <code>25000</code>\n\n"
        "💡 <i>Цена должна быть от 100 до 1 000 000 сом</i>",
...
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_price")
def handle_price_input(message):
    ...
    if not is_valid:
        safe_send_message(user_id,
            ...
            "Введите число от 100 до 1 000 000 сом.\n"
            "Например: <code>25000</code>\n\n"
            "💡 <i>Используйте только цифры</i>",
            reply_markup=get_cancel_keyboard()
        )
        return
    
    # Сохраняем цену в черновик
...
@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_city")
def handle_city_input(message):
    ...
    # Сохраняем город в черновик
    AdDraftManager.update_draft(user_id, 'city', city_text)
    
    # Переходим к вводу метро
    UserState.set_state(user_id, "entering_metro")
    
    safe_send_message(user_id,
        f"📍 <b>Город сохранен:</b> {city_text}\n\n"
        "🚇 <b>Укажите станцию метро или ближайший ориентир:</b>\n\n"
        "Например: <code>Аламедин-1</code>, <code>Беш-Кунгей</code>\n"
        "Или напишите <code>нет</code>, если нет метро.",
        reply_markup=get_cancel_keyboard()
    )

@bot.message_handler(func=lambda m: UserState.get_state(m.from_user.id) == "entering_metro")
def handle_metro_input(message):
    """Обработка ввода метро"""
    user_id = message.from_user.id
    metro_text = message.text.strip()
    
    if metro_text == "❌ Отмена":
        UserState.set_state(user_id, None)
        ensure_main_keyboard(user_id)
        return
    
    # Если пользователь написал "нет" или оставил пустым, сохраняем как "Нет"
    if metro_text.lower() == 'нет' or not metro_text:
        metro_text = 'Нет'
    
    # Сохраняем метро в черновик
    AdDraftManager.update_draft(user_id, 'metro', metro_text)
    
    # Переходим к загрузке фото
    UserState.set_state(user_id, "uploading_photos")
    
    safe_send_message(user_id,
        f"🚇 <b>Метро/ориентир сохранен:</b> {metro_text}\n\n"
        "📸 <b>Теперь загрузите фотографии телефона:</b>\n\n"
        f"• Минимум: {MIN_PHOTOS} фото\n"
        f"• Максимум: {MAX_PHOTOS} фото\n\n"
        "📌 <b>Советы для фото:</b>\n"
        "1. Сделайте фото с разных ракурсов\n"
        "2. Покажите экран и заднюю панель\n"
        "3. Сфотографируйте возможные повреждения\n"
...
def start_ad_creation(update):
    ...
    if hasattr(update, 'message'):
        ...
        logger.info(f"Запрос создания объявления от {user_id} через текстовую кнопку")
    else:
        user_id = update.from_user.id
        logger.info(f"Запрос создания объявления от {user_id} через callback кнопку")
        bot.answer_callback_query(update.id)
    
    # ОЧИСТКА: Полностью сбрасываем предыдущее состояние
    clear_ad_creation_state(user_id)
    
    # Проверяем, есть ли незавершенный черновик (после очистки его не должно быть)
    draft = AdDraftManager.get_draft(user_id)
    if draft:
...
def search_model_handler(call):
    """Обработка поиска модели - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = call.from_user.id
...
def admin_command(message):
    """Команда администратора - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = message.from_user.id
...
"""
    ...
    bot.send_message(user_id, admin_text, reply_markup=keyboard)

@bot.message_handler(commands=['stats'])
def stats_command(message):
    """Подробная статистика - ДОБАВЛЯЕМ КНОПКУ НАЗАД"""
    user_id = message.from_user.id
...
def publish_ad_handler(call):
    ...
    try:
        # Форматируем текст объявления для канала
        premium_marker = '💎 <b>ПРЕМИУМ ОБЪЯВЛЕНИЕ</b>\n\n' if ad.get('is_premium') else ''
        ad_text = f"""{premium_marker}📱 <b>Модель:</b> {ad.get('model', 'Не указано')}
...
"""
        ...
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            types.InlineKeyboardButton("📞 Связаться", callback_data=f"contact_seller:{ad_id}"),
            types.InlineKeyboardButton("📱 Поделиться контактом", callback_data=f"share_contact:{ad_id}")
        )
...
# ===== ДОБАВЛЯЕМ ОБРАБОТЧИК ДЛЯ ВСЕХ CALLBACK-КНОПОК С НАЗАД =====

@bot.callback_query_handler(func=lambda call: True)
def handle_all_callbacks(call):
    """Обработка всех callback-кнопок - ДОБАВЛЯЕМ ОБРАБОТКУ UNIVERSAL_BACK"""
    try:
        ...
        if call.data == "faq":
            show_faq(call)
        elif call.data == "universal_back":
            universal_back_handler(call)
        elif call.data == "buy_premium":
            buy_premium(call)
        elif call.data == "photos_done":
            photos_done_handler(call)
        elif call.data == "simple_donate":
            simple_donate_handler(call)
        elif call.data == "my_payments":
            my_payments_handler(call)
            ...
            confirm_broadcast_handler(call)
        elif call.data == "cancel_broadcast":
            cancel_broadcast_handler(call)
        elif call.data == "back":
            back_handler(call)
        elif call.data.startswith('models_page:'):
            handle_models_pagination(call)
        elif call.data == "search_model":
            search_model_handler(call)
        elif call.data.startswith('model:'):
            select_model_handler(call)
        elif call.data.startswith('condition:'):
            select_condition_handler(call)
        elif call.data.startswith('memory:'):
            select_memory_handler(call)
        elif call.data.startswith('box:'):
            select_box_handler(call)
        elif call.data.startswith('docs:'):
            select_docs_handler(call)
        elif call.data.startswith('accessories:'):
            select_accessories_handler(call)
        elif call.data.startswith('donate_amount:'):
            fixed_donate_amount_handler(call)
        elif call.data == "enter_donate_amount":
            enter_donate_amount_handler(call)
        elif call.data == "back_to_donate":
            back_to_donate_handler(call)
        elif call.data.startswith('check_user_payment:'):
            check_user_payment_handler(call)
        elif call.data.startswith('support_done:'):
            support_done_handler(call)
        elif call.data == "add_more_photos":
            add_more_photos_handler(call)
        elif call.data == "publish_ad":
            publish_ad_handler(call)
        elif call.data == "back_to_preview":
            # Обработчик возврата к превью
            user_id = call.from_user.id
            show_ad_preview(user_id)
            bot.answer_callback_query(call.id)
        else:
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("✅ Универсальная кнопка 'Назад' добавлена во все разделы!")
    print("✅ Все клавиатуры модифицированы для включения кнопки 'Назад'")
    print("✅ Обработчики универсального возврата добавлены и настроены")
    print("✅ Кнопка 'Создать объявление' полностью восстановлена и оптимизирована!")
    print("✅ Обработчики для ввода объема памяти и цвета телефона добавлены и работают!")
    print("=" * 60)
    print("🚀 Запуск бота...")
