...
ADMIN_CEO_ID = os.getenv("7577716374", "")  # ID или username CEO
ADMIN_SUPPORT_ID = os.getenv("6764228404", "")  # ID или username Поддержки
...
def get_admin_keyboard(user_id):
    ...
    keyboard.add(
        types.InlineKeyboardButton("📝 Ответить", callback_data=f"admin_reply:{user_id}"),
        types.InlineKeyboardButton("✅ Решено", callback_data=f"admin_solved:{user_id}"),
        types.InlineKeyboardButton("⏳ На рассмотрении", callback_data=f"admin_pending:{user_id}")
    )
    return keyboard
...
try:
    ...
    CRYPTO_BOT_API_KEY = os.getenv("CRYPTO_BOT_API_KEY", "51750:AAt1ktPvx7WIx0rhhzuSm21sTDZPT66sGLl")
...
def handle_support_message(message):
    """Обработка сообщения в поддержку - с уведомлением администраторов"""
    user_id = message.from_user.id
    ...
    # Формируем сообщение для администраторов согласно ТЗ
    admin_message_text = f"""
<b>Новое обращение в поддержку</b>

👤 <b>Пользователь:</b> {first_name} {last_name}
🔗 <b>Username:</b> @{username if username != 'нет' else 'не указан'}
🕐 <b>Дата и время:</b> {datetime.now().strftime('%d.%m.%Y %H:%M')}
...
"""
    ...
    if ADMIN_CEO_ID:
        try:
            ...
            storage.admin_messages[(ADMIN_CEO_ID, admin_msg.message_id)] = user_id
            admins_notified.append("CEO")
            logger.info(f"Уведомление отправлено CEO ({ADMIN_CEO_ID})")
        except Exception as e:
            logger.error(f"Ошибка отправки уведомления CEO: {e}")
    
    # Отправляем Поддержке
    if ADMIN_SUPPORT_ID and ADMIN_SUPPORT_ID != ADMIN_CEO_ID:
        try:
            admin_msg = bot.send_message(
                ADMIN_SUPPORT_ID,
                admin_message_text,
            ...
            storage.admin_messages[(ADMIN_SUPPORT_ID, admin_msg.message_id)] = user_id
            admins_notified.append("Поддержка")
            logger.info(f"Уведомление отправлено Поддержке ({ADMIN_SUPPORT_ID})")
        except Exception as e:
    ...
    safe_send_message(user_id,
        ...
        "Наш менеджер ответит вам в течение 24 часов.\n\n"
        f"📋 <i>Уведомления отправлены: {', '.join(admins_notified) if admins_notified else 'администраторам'}</i>"
    )
...
def update_admin_keyboard_status(admin_id, message_id, user_id, action):
    """Обновляет статус кнопок у администратора"""
    try:
        # Получаем текущее сообщение
        message = storage.support_messages.get(user_id, {})
        if not message:
            return
        
        first_name = message.get('first_name', 'Пользователь')
        username = message.get('username', 'нет')
        timestamp = message.get('timestamp', datetime.now()).strftime('%d.%m.%Y %H:%M')
...
"""
        ...
        logger.error(f"Ошибка обновления клавиатуры админа: {e}")
...
def admin_callback_handler(call):
    ...
    if user_id not in storage.support_messages:
        bot.answer_callback_query(call.id, "❌ Обращение не найдено или уже обработано", show_alert=True)
        return
    ...
    if action == "admin_reply":
        ...
        bot.send_message(
            admin_id,
            "✏️ <b>Введите текст ответа пользователю:</b>\n\n"
        ...
        try:
            bot.send_message(
                ...
                "Ваш вопрос решён. Если у вас возникнут новые вопросы, "
                "обращайтесь в поддержку.",
                reply_markup=get_main_keyboard()
            )
            logger.info(f"Вопрос пользователя {user_id} помечен как решенный")
        except Exception as e:
            logger.error(f"Ошибка отправки сообщения пользователю {user_id}: {e}")
        
        # Удаляем клавиатуру у всех администраторов
        for (a_id, msg_id), u_id in list(storage.admin_messages.items()):
            if u_id == user_id:
                try:
                    update_admin_keyboard_status(a_id, msg_id, user_id, "✅ Решено")
                    # Удаляем из отслеживания
                    if (a_id, msg_id) in storage.admin_messages:
                        del storage.admin_messages[(a_id, msg_id)]
                except Exception as e:
                    logger.error(f"Ошибка обновления сообщения админа {a_id}: {e}")
        
        # Удаляем обращение из хранилища
        if user_id in storage.support_messages:
            del storage.support_messages[user_id]
        
        bot.answer_callback_query(call.id, "✅ Вопрос помечен как решенный")
    
    elif action == "admin_pending":
        # Отправляем пользователю сообщение о рассмотрении
        try:
            bot.send_message(
                user_id,
                "⏳ <b>Ваш вопрос на стадии рассмотрения.</b>\n\n"
                "Пожалуйста, ожидайте ответа от техподдержки.",
                reply_markup=get_main_keyboard()
            )
            logger.info(f"Вопрос пользователя {user_id} помечен как на рассмотрении")
        except Exception as e:
            logger.error(f"Ошибка отправки сообщения пользователю {user_id}: {e}")
        ...
        for (a_id, msg_id), u_id in list(storage.admin_messages.items()):
            if u_id == user_id:
                try:
                    update_admin_keyboard_status(a_id, msg_id, user_id, "⏳ На рассмотрении")
                    # Удаляем из отслеживания
                    if (a_id, msg_id) in storage.admin_messages:
                        del storage.admin_messages[(a_id, msg_id)]
                except Exception as e:
        ...
        if user_id in storage.support_messages:
            storage.support_messages[user_id]['status'] = 'pending'
        
        bot.answer_callback_query(call.id, "⏳ Вопрос помечен как на рассмотрении")
...
def handle_admin_reply(message):
    ...
    # Проверяем существование обращения
    if user_id not in storage.support_messages:
        bot.send_message(admin_id, "❌ Обращение не найдено или уже обработано.")
        if admin_id in storage.admin_reply_context:
            del storage.admin_reply_context[admin_id]
        return
...
---
<i>Если у вас остались вопросы, вы можете снова обратиться в поддержку.</i>
"""
        
        bot.send_message(user_id, response_text, reply_markup=get_main_keyboard())
        logger.info(f"Ответ администратора {admin_id} отправлен пользователю {user_id}")
        
        # Уведомляем администратора
        bot.send_message(admin_id, "✅ <b>Ответ успешно отправлен пользователю!</b>")
        
        # Обновляем статус у всех администраторов
        for (a_id, msg_id), u_id in list(storage.admin_messages.items()):
            if u_id == user_id:
                try:
                    update_admin_keyboard_status(a_id, msg_id, user_id, "💬 Отвечено")
                    # Удаляем из отслеживания
                    if (a_id, msg_id) in storage.admin_messages:
                        del storage.admin_messages[(a_id, msg_id)]
                except Exception as e:
...
if __name__ == '__main__':
    ...
    print("✅ Административный функционал полностью удален!")
    print("✅ Сохранены: донат, поддержка, FAQ")
    print("=" * 60)
    print("✅ Изменения согласно ТЗ:")
    print("• Добавлена обработка кнопки 'Назад' в разделе доната")
    print("• Удалена кнопка 'Проверить оплату' из раздела доната")
    print("• Добавлена расширенная система поддержки с FSM")
    print("• Добавлены администраторы: CEO и Поддержка")
    print("• Добавлены Inline-кнопки для администраторов")
    print("• Реализованы состояния: ожидание поддержки, ожидание ответа")
    print("• Добавлена валидация прав администраторов")
    print("=" * 60)
    print("🚀 Запуск бота...")
