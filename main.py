...
# ВАЖНО: УДАЛЕНЫ ОБРАБОТЧИКИ ДЛЯ КНОПОК, КОТОРЫЕ БЫЛИ УДАЛЕНЫ СОГЛАСНО ТЗ:
# 1. create_ad_callback (для кнопки "Создать объявление")
# 2. regular_mode_callback (для кнопки "Обычное создание")
# 3. edit_field:model и другие, если они относятся к обычному режиму
