...
ADMIN_SUPPORT_ID = os.getenv("6764228404", "6764228404")  # ID или username Поддержки
        ...
        if user_id in self.user_last_tickets:
            for ticket_id in self.user_last_tickets[user_id][-5:]:  # Проверяем последние 5 тикетов
                ...
                if ticket and ticket['status'] in ['new', 'pending']:
                    # Простая проверка схожести по ключевых словам
...
# ===== ФУНКЦИИ ДЛЯ РАБОТЫ С КОНТАКТАМИ И КНОПКАМИ =====
def create_contact_keyboard(contact_types, contact_values):
    """Создает инлайн-клавиатуру с контактными кнопками согласно ТЗ"""
    markup = types.InlineKeyboardMarkup()
    
    # Если contact_types - строка, преобразуем в список
    if isinstance(contact_types, str):
        contact_types = [contact_types]
    
    # Если contact_values - строка, создаем список с одним значением
    # Для нескольких контактов предполагается список значений
    if isinstance(contact_values, str):
        contact_values = [contact_values] * len(contact_types)
    
    # Создаем кнопки в зависимости от типа контакта
    for i, contact_type in enumerate(contact_types):
        if contact_type == 'phone' and i < len(contact_values):
            phone = contact_values[i].replace(' ', '').replace('+', '')
        ...
        elif contact_type == 'username' and i < len(contact_values):
            username = contact_values[i].replace('@', '')
            btn = types.InlineKeyboardButton(
                "💬 Написать в чат", 
...
def publish_announcement(data):
    """
    Функция публикации объявления согласно ТЗ
    Принимает словарь data с полями:
    - text: текст объявления
    - photo: file_id или URL фото (опционально)
    - contact_type: строка или список типов контактов
    - contact_value: строка или список значений контактов
    """
    try:
        # Получаем данные
        text = data.get('text', '')
        ...
        # Формируем клавиатуру с контактами
        markup = create_contact_keyboard(contact_type, contact_value)
...
def format_ad_text(ad_data, preview=False):
    """Форматирование текста объявления с HTML-разметкой"""
...
"""
    ...
    contact_type = ad_data.get('contact_type')
    contact_value = ad_data.get('contact', 'Не указан')
    
    if contact_type == 'phone':
        text += f"• Телефон: {contact_value}\n"
    elif contact_type == 'telegram' or contact_type == 'username':
        text += f"• Telegram: {contact_value}\n"
    else:
...
def handle_telegram_contact(message):
    ...
    set_ad_state(user_id, "photos", {"contact_type": "username", "contact": contact_info})
...
def publish_ad_callback(call):
    """Публикация объявления с инлайн-кнопками для контактов"""
    user_id = call.from_user.id
    ad_step, ad_data, ad_photos = get_ad_state(user_id)
    
    # Получаем данные контактов из ad_data
    contact_type = ad_data.get('contact_type')
    contact_value = ad_data.get('contact')
    
    # Если нет контактных данных, используем данные пользователя
    if not contact_type and not contact_value:
        contact_type = 'username'
        contact_value = call.from_user.username or call.from_user.first_name
    
    # Формируем финальный текст
    final_text = format_ad_text(ad_data, preview=False)
    
    # Создаем инлайн-клавиатуру для контактов
    contact_markup = create_contact_keyboard(contact_type, contact_value)
    ...
    try:
        if ad_photos:
            ...
            for i, photo_id in enumerate(ad_photos):
                if i == 0:
                    # Для первого фото добавляем подпись и клавиатуру
            ...
            # Если есть контактные кнопки, отправляем отдельное сообщение с ними
            if contact_markup.keyboard:
                bot.send_message(
                    CHANNEL_ID, 
            ...
            # Если нет фото, отправляем только текст с контактными кнопками
            bot.send_message(
                CHANNEL_ID, 
                final_text, 
                parse_mode="HTML", 
        ...
        bot.answer_callback_query(
            call.id, 
            "❌ Ошибка публикации. Попробуйте позже.", 
            show_alert=True
        )
...
if __name__ == '__main__':
    ...
    print("✅ Публикация в канал с инлайн-кнопками для связи")
