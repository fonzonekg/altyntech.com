...
def simple_donate_handler(call):
    ...
    # Создаем inline-клавиатуру с кнопкой указания суммы
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(
    ...
    keyboard.add(
        types.InlineKeyboardButton("⬅️ Назад", callback_data="back_to_donate")
    )
...
def my_payments_handler(call):
    ...
    bot.send_message(user_id, payments_text)
    bot.answer_callback_query(call.id)
...
def handle_all_callbacks(call):
    ...
    try:
        ...
        if call.data == "faq":
            ...
            buy_premium(call)
        elif call.data == "photos_done":  # Добавлено: обработка кнопки "Достаточно"
            photos_done_handler(call)
        elif call.data == "simple_donate":  # Добавлено: обработка кнопки "Просто поддержать"
            simple_donate_handler(call)
        elif call.data == "my_payments":  # Добавлено: обработка кнопки "Мои платежи"
