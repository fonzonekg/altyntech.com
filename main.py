...
Всё, чтобы подать объявление без лишних сложностей!"""
    ...
    # Добавляем inline-кнопку для создания объявления
    try:
        ...
        bot.send_message(
            user_id,
            "Хотите создать объявление о продаже смартфона?",
            reply_markup=keyboard
        )
    except Exception as e:
        logger.error(f"Ошибка отправки кнопки создания объявления: {e}")
...
def handle_iphone_model(message):
    ...
    if not model:
        bot.send_message(user_id, "❌ Пожалуйста, введите модель iPhone.")
        return
...
def handle_iphone_battery(message):
    ...
    try:
        ...
        if 70 <= battery <= 100:
            ...
            bot.send_message(user_id, "❌ Введите число от 70 до 100:")
    except ValueError:
        bot.send_message(user_id, "❌ Пожалуйста, введите число от 70 до 100:")
...
def handle_iphone_color(message):
    ...
    if not color:
        bot.send_message(user_id, "❌ Пожалуйста, введите цвет.")
        return
...
def handle_android_model(message):
    ...
    if not model:
        bot.send_message(user_id, "❌ Пожалуйста, введите модель.")
        return
    ...
    bot.send_message(
        user_id,
        "🧠 Выберите оперативную память (RAM):",
        reply_markup=keyboard
    )
...
def handle_android_processor(message):
    ...
    if not processor:
        bot.send_message(user_id, "❌ Пожалуйста, введите модель процессора.")
        return
...
def handle_price_usd(message):
    ...
    try:
        ...
        if price_usd <= 0:
            bot.send_message(user_id, "❌ Цена должна быть больше 0. Введите снова:")
            return
        ...
        bot.send_message(user_id, "❌ Пожалуйста, введите число. Например: <code>500</code>", parse_mode="HTML")
...
def handle_price_kgs(message):
    ...
    try:
        ...
        bot.send_message(user_id, "❌ Пожалуйста, введите число. Например: <code>50000</code>", parse_mode="HTML")
...
def edit_ad_callback(call):
    ...
    keyboard.row(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_preview"))
