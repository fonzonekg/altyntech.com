...
Всё, чтобы подать объявление без лишних сложностей!"""
    ...
    # Добавляем выбор режима создания
    keyboard = types.InlineKeyboardMarkup()
    keyboard.row(
        types.InlineKeyboardButton("🤖 Умное создание", callback_data="smart_create_ad"),
        types.InlineKeyboardButton("➕ Обычное создание", callback_data="create_ad")
    )
    
    bot.send_message(
        user_id,
        "🎯 <b>Выберите режим создания объявления:</b>\n\n"
        "🤖 <b>Умное создание:</b>\n"
        "• Автоподсказки моделей\n"
        "• Предсказание следующих шагов\n"
        "• Проверка на дубликаты\n"
        "• Валидация данных\n\n"
        "➕ <b>Обычное создание:</b>\n"
        "• Стандартный пошаговый процесс",
        parse_mode="HTML",
        reply_markup=keyboard
    )
...
def smart_create_ad_callback(call):
    ...
    keyboard.row(types.InlineKeyboardButton("🔙 Обычный режим", callback_data="regular_mode"))
    keyboard.row(types.InlineKeyboardButton("❌ Отмена", callback_data="cancel_ad"))
...
def show_smart_next_step(user_id, chat_id=None, message_id=None):
    ...
    if next_step == "model":
        ...
        show_step_interface(user_id, next_step, chat_id, message_id)
        return
...
def continue_draft_callback(call):
    ...
    if ad_step.startswith("smart_"):
        ...
        show_step_interface(user_id, ad_step, call.message.chat.id, call.message.message_id)
...
@bot.callback_query_handler(func=lambda call: call.data == "regular_mode")
def regular_mode_callback(call):
    """Переключение на обычный режим"""
    user_id = call.from_user.id
    clear_ad_state(user_id)
    
    # Начинаем обычное создание
    create_ad_callback(call)
