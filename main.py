...
def predict_next_steps(ad_data):
    ...
    if model and device_type == "iphone":
        if not ad_data.get('memory'):
            ...
            steps.append(("battery", "Введите состояние аккумулятора (0-100%)"))
    ...
    if model and (ad_data.get('condition') or ad_data.get('package')):
    ...
    if model and ad_data.get('price_kgs'):
...
def cancel_command(message):
    ...
    # Вместо отправки сообщения просто возвращаем основную клавиатуру
    ensure_main_keyboard(user_id)
...
def smart_create_ad_callback(call):
    ...
    # УДАЛЕНО: safe_send_message с текстом "Начинаем создание объявления..."
...
def cancel_ad_callback(call):
    ...
    # УДАЛЕНО: safe_send_message с текстом "Возвращаю в главное меню"
...
def show_smart_next_step(user_id, chat_id=None, message_id=None):
    ...
    logger.info(f"show_smart_next_step: user_id={user_id}, ad_step={ad_step}, device_type={ad_data.get('device_type')}")
    
    # Получаем предсказанные следующие шаги
    next_steps = predict_next_steps(ad_data)
    logger.info(f"Предсказанные шаги: {next_steps}")
    ...
    if next_step == "model":
        ...
        # Создаем клавиатуру с правильными callback_data
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        for i in range(0, len(options), 2):
            if i + 1 < len(options):
                keyboard.row(
        ...
        # Важно: Устанавливаем правильное состояние
...
def smart_back_callback(call):
    ...
    # Инициализируем историю шагов, если её нет
    ...
    if ad_step:
        step_history.append(ad_step)
    
    # Определяем предыдущий шаг на основе текущего состояния
    if "model" in ad_step:
        ...
        if ad_data.get('device_type') == "iphone":
...
def smart_iphone_condition_callback(call):
    ...
    # Устанавливаем следующее состояние для батареи
    set_ad_state(user_id, "smart_iphone_battery", ad_data)
