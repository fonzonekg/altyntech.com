...
def simple_donate_handler(call):
    """Обработка простого доната без активации премиума"""
    user_id = call.from_user.id
    
    # Создаем инвойс с минимальной суммой (1 USDT ≈ 100 сом)
    invoice = CryptoBotAPI.create_invoice(
        amount=1,  # 1 USDT
        currency="USDT",
    ...
    if invoice:
        ...
        bot.send_message(
            ...
            "Ваша поддержка поможет улучшать функционал бота и добавлять новые возможности.\n\n"
            "Ссылка для оплаты действительна 30 минут.",
            reply_markup=keyboard
        )
        
        bot.answer_callback_query(call.id, "✅ Счет для поддержки создан")
    else:
        bot.answer_callback_query(call.id, 
            "❌ Ошибка создания счета. Попробуйте позже.", 
            show_alert=True)
